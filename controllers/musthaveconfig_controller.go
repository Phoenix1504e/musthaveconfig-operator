...
func (r *MustHaveConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// 1. Load the MustHaveConfig object that triggered this reconcile
	...
	// 2. Determine desired ConfigMap location and values
	...
	if desiredValue == "" {
		mhc.Status.Synced = false
		mhc.Status.Message = "spec.value is empty"
		_ = r.Status().Update(ctx, &mhc)
		return ctrl.Result{}, nil
	}
	...
	if apierrors.IsNotFound(err) {
		...
		// Set owner reference so ConfigMap is deleted with the CR
		...
		// Some other error while fetching ConfigMap
		return ctrl.Result{}, err
	} else {
		// ConfigMap exists -> ensure it has the desired data
		if cm.Data == nil {
			cm.Data = map[string]string{}
		}
		currentValue, ok := cm.Data[desiredKey]
	...
	// 4. Update Status to reflect that everything is in sync
	...
	if err := r.Status().Update(ctx, &mhc); err != nil {
		...
		// not fatal; we don't requeue just for this
	}
